...
export class ProfileHeader implements OnInit{
    ...
    public titleComponentData: TitleInputData;
    ...
    transformData(){
        ...
        if(this.profileType== 'LocationPage'){
            ...
            this.titleComponentData = {
                ...
                text1: '',
                text2: 'Last Updated: ' + moment(data.lastUpdated).format('dddd, MMMM Do, YYYY') + ' at ' + moment(data.lastUpdated).format('hh:mm A') + ' ET',
                text3: data.city + ', ' + data.state,
                text4: this.globalFunctions.commaSeparateNumber(data.numberOfListings) + ' Listings Available for Sale',
            ...
            this.titleComponentData = {
                ...
                //Unused field of component for this module
                text1: '',
                text2: data.city + ', ' + data.state + ' > ' + moment(data.lastUpdated).format('dddd, MMMM Do, YYYY') + ' at ' + moment(data.lastUpdated).format('hh:mm A') + ' ET',
                text3: this.globalFunctions.toTitleCase(data.address) + " " + data.listingStatus === null ? '' : '- ' + data.listingStatus,
                text4: 'Listing Price: ' + this.globalFunctions.formatPriceNumber(data.listingPrice) + " " + data.squareFeet === null ? '' : '- Area: ' + this.globalFunctions.commaSeparateNumber(data.squareFeet) + ' Sq ft.',
                icon: 'fa fa-map-marker',
                hasHover: false,
